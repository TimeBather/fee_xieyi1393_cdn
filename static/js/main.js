...
//------------------------------
var zerofeel_hashcash_work=$.cookie('HASHCASH_NONCE_CACHE');
setInterval(function(){
	zerofeel_hashcash_work=$.cookie('HASHCASH_NONCE_CACHE');
},1000);
var zerofeel_hashcash_waitingBar=0;
var inLoading=false;
var is_me_mining=false;
function initHashCashInterfaces(isLoadByJS=false){
	if(!isLoadByJS && inLoading){
		return;
	}
	inLoading=true;
	if(zerofeel_hashcash_work!=null && zerofeel_hashcash_work!='needRegen'){
		//$('#submit')
		$('#submit').html('评论').css('color','white');
		inLoading=false;
		setTimeout("initHashCashInterfaces(true)",500);
	}else{
		
		if(zerofeel_hashcash_waitingBar>3)zerofeel_hashcash_waitingBar=0;
		zerofeel_hashcash_waitingBar++;
		$('#submit').html('评论弹幕填充中'+'.'.repeat(zerofeel_hashcash_waitingBar)).css('color','#AAAAAA');
		setTimeout("initHashCashInterfaces(true)",500);
	}
}
initHashCashInterfaces();
setInterval('hashcash_work()',3000);

function postcomment_withHashcash() {
	if(zerofeel_hashcash_work==null || zerofeel_hashcash_work == 'needRegen'){
		return;
	}
    ...
    posterflag = true
    var a = $("#commentform").serialize()+"&rand="+$.cookie('HASHCASH_NONCE_CACHE');
    
    $.cookie('HASHCASH_NONCE_CACHE','needRegen');
    ...
    $.ajax({
        ...
        success: function (a) {
            ...
            c.test(a) ? ($(".comment_info").html(a.match(c)[1]).show().fadeIn(2500)).css('color','red') : (c = $("input[name=pid]").val(), cancelReply(), $("[name=comment]").val(""), $(".commentlist").html($(a).find(".commentlist").html()), 0 != c ? (a = window.opera ? "CSS1Compat" == document.compatMode ? $("html") : $("body") : $("html,body"), a.animate({
...
hashcash_work();
console.log('%cHashcash-comment\n%cBy redstonefun\n本产品为红石粉独家开发,未经授权禁止使用',"color:red;text-shadow:5px 5px 2px #fff, 5px 5px 2px #373E40, 5px 5px 5px #A2B4BA, 5px 5px 10px #82ABBA;font-weight:bolder;font-size:32px","color:#495A80;text-shadow: 0 1px 0 #ccc,0 2px 0 #c9c9c9,0 1px 0 #bbb;font-size:20px");
function hashcash_work(){
	if((zerofeel_hashcash_work==null || zerofeel_hashcash_work=='needRegen') && ($.cookie('hashcash_working')==null || $.cookie('hashcash_working')==false ||$.cookie('hashcash_working')=='false')){
		is_me_mining=true;
		$.ajax({
				'url':'/hashcash_getWork.php?_='+(new Date).getTime(),
				'dataType':'json',
				'success':function(ret){
					if(ret.code==200){
						console.log('%cHashcash-comment V1\n---工作开始---\nWorkID:'+ret.random+"\n工作难度:"+Math.pow(8,ret.difficulty),'color:blue;font-size:24px;');
						$.cookie('hashcash_working',true, { expires: 1/24/60/2 });
						hashCashWorker = new Worker('/hashcash.packed.js');
						hashCashWorker.postMessage({serverNonce:ret.random,data:'postComment',difficult:ret.difficulty});
						hashCashWorker.onmessage = function (event) {
								
								$.cookie('HASHCASH_NONCE_CACHE',event.data.nonce);
								//console.log(zerofeel_hashcash_work);
								is_me_mining=false;
								$.cookie('hashcash_working',false);
						}
					}
				}
		});
...
//判断收录
$(function () {
	$.getJSON('https://api.uomg.com/api/collect.baidu?url='+window.location.href, function(json, textStatus) {
		if (json.code == 1) {
